...
export function ImportStudentsModal({ onImport }: ImportStudentsModalProps) {
    ...
    <Dialog onOpenChange={(open) => !open && setStatus("idle")}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecciona un archivo .xlsx o .csv con la lista de estudiantes para el registro masivo.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          {status === "success" && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-success/10 text-success border border-success/20 animate-in fade-in zoom-in duration-300">
              ...
              <p className="text-sm font-medium">¡Éxito! Se han importado {rowCount} estudiantes correctamente.</p>
            </div>
          )}
        ...
        <DialogFooter>
          ...
          >
            {status === "uploading" ? "Procesando..." : status === "success" ? "Carga Finalizada" : "Cargar Estudiantes"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
